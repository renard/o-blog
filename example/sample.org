...
首先，你需要安装有 [[http://www.gnu.org/s/emacs][emacs]] 和 [[http://orgmode.org/][org-mode]] ，如果没有这些程序，你应该先安装它们。
...
1. [X] 中文文件名无法正常生成 --> 使用 MD5 取代 sanitize-string
   * 现在，使用 =#+FILENAME_SANITIZER: md5= 代替。
2. [X] pull upstream
...
- 外观
  - 带[[file:{lisp}(ob:path-to-root){/lisp}/debug.html][调试功能]]的动态模版系统 (使用 lisp 函数)
  - 输出的代码片断带有语法加亮
